    ...
    try {
      ...
      if (student) {
        ...
        if (!Object.keys(types).some((value) => value === type)) {
          return res.json({
            messages: [{ variant: "error", text: "type valid is required" }],
          });
        }
        ...
        if (getPerson) {
          if (type === "addressInfo") {
            ...
            if (extraId !== null && isNaN(extraId)) {
              ...
              return resUpdate
                ? res.json({
                    messages: [{ variant: "success", text: "Student saved" }],
                  })
                : res.json({
            ...
            if (
              ...
              return res.json({
                messages: [
                  {
                    variant: "error",
                    text: "Student information record requires a first name.",
                  },
                ],
              });
            }
            ...
            if (
              ...
              return res.json({
                messages: [
                  {
                    variant: "error",
                    text: "Student information record requires a last name.",
                  },
                ],
              });
            }
...
studentRouter.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const person: any = await db("sfa.person")
        .where({ id: person_id })
        .first();
...
studentRouter.post("/search", async (req: Request, res: Response) => {
  let { terms } = req.body;
  ...
  if (terms.length == 0)
    res.json({
      data: [],
      messages: [{ variant: "error", text: "You must include search terms" }],
    });

  let termParts = terms.split(/[,-\s]/);
  termParts = termParts.filter((t: string) => t.trim().length > 0);

  let kq = db("sfa.student").join(
    "sfa.person",
    "student.person_id",
    "person.id"
  );
  ...
  for (let r of results) {
    ...
    let history = await db("sfa.application")
...
studentRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (student) {
        const resInsert = await db("sfa.education").insert({
          ...data,
          student_id,
        });
      ...
      return res
        .status(404)
        .send({ messages: [{ variant: "error", text: "Failed" }] });
    } catch (error) {
      console.error(error);
      return res
        .status(400)
        .send({ messages: [{ variant: "error", text: "Failed", error }] });
    }
  }
);
    ...
    try {
      ...
      if (student) {
        if (!Object.keys(data).some((value) => value === "sin")) {
          ...
          if (sinList.length > 0) {
            return res.json({
              messages: [{ variant: "error", text: "SIN is in use." }],
            });
          }
        }
        const table = types[type];
    ...
    try {
      ...
      const student: any = await db("sfa.student")
        ...
        .select(
          "sfa.student.*",
          db.raw(
            "sfa.fn_get_pre_leg_sta_up_weeks(student.id) AS pre_leg_sta_up_weeks"
          ),
          db.raw(
            "sfa.fn_get_pre_leg_outside_travel(student.id) AS pre_leg_outside_travel"
          )
        )
        .first();

      if (student) {
        const person = await db("sfa.person")
        ...
        if (person) {
          ...
          const consentInfo = await db("sfa.student_consent").where({
            student_id: id,
          });

          const residenceInfo = await db("sfa.residence").where({
            student_id: id,
          });
...
studentRouter.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        Object.keys(data).some((k) => k === "consent_person") &&
        !(data.consent_person?.trim().length > 2)
      ) {
        return res.json({
          messages: [{ variant: "error", text: "Consent Person is required" }],
        });
      }
...
studentRouter.post(
  ...
  [
    param("student_id").isInt().notEmpty(),
    body("data.start_academic_year_id")
      .notEmpty()
      .withMessage("Start academic year is required"),
    body("data.end_academic_year_id")
      .notEmpty()
      .withMessage("End academic year is required"),
    body("data.consent_person")
      .isString()
      .isLength({ min: 3 })
      .withMessage("Consent person is required"),
  ],
  ReturnValidationErrorsCustomMessage,
  async (req: Request, res: Response) => {
    try {
      ...
      if (student) {
        ...
        const resInsert = await db("sfa.student_consent").insert({
          ...data,
          student_id,
        });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (type !== "dependent" && type !== "d_eligibility") {
        return res.json({
          messages: [{ variant: "error", text: "Type is required" }],
        });
      }
      ...
      if (student && application) {
        const table =
          type === "dependent" ? "sfa.dependent" : "sfa.dependent_eligibility";
        const resUpdate = await db(table)
          .where({ id })
          .update({ ...data });
      ...
      if (student) {
        await db.transaction(async (trx) => {
          ...
          if (resInsertDependent) {
            ...
            return res.json({
              messages: [{ variant: "error", text: "Failed" }],
            });
          }
        });
      } else {
      ...
      if (verifyRecord && verifyRecord.dependent_id !== Number(dependent_id)) {
        return res.status(404).send({
          messages: [
            {
              variant: "success",
              text: "The relationship between the data does not exist.",
            },
          ],
        });
      }

      const deleteRecordDEligibility: any = await db(
        "sfa.dependent_eligibility"
      )
        .where({ id: d_elegibility_id })
        .del();
...
studentRouter.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        Object.keys(data).some((k) => k === "is_in_progress") &&
        !data.is_in_progress
      ) {
      ...
      if (student) {
        const residence: any = await db("sfa.residence")
          .where({ id: extraId })
          .first();

        if (Object.keys(data).some((k) => k === "from_month")) {
          if (
            Number(data.from_month) > residence.to_month &&
        ...
        if (Object.keys(data).some((k) => k === "to_year")) {
          if (
            residence.from_year === Number(data.to_year) &&
        ...
        if (Object.keys(data).some((k) => k === "from_year")) {
          if (
            residence.to_year === Number(data.from_year) &&
            residence.from_month > residence.to_month
          ) {
            data.to_month = null;
          }
        ...
        if (
          ...
          const startYear = Object.keys(data).some((k) => k === "from_year")
            ? data.from_year
            : residence.from_year;
          ...
          const endYear = Object.keys(data).some((k) => k === "to_year")
            ? data.to_year
            : residence.to_year;
          ...
          if (regex.test(startDate) && regex.test(endDate)) {
            ...
            const filterResidences = residences.filter((residence: any) => {
              return (
                regex.test(residence.startDate) && regex.test(residence.endDate)
              );
            });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (student) {
        ...
        const resInsert = await db("sfa.residence").insert({
          ...data,
          student_id,
        });
...
studentRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    let history_details = await db("sfa.history_detail")
      .innerJoin(
        "sfa.institution",
        "history_detail.institution_id",
        "institution.institution_id"
      )
      .innerJoin(
        "sfa.study_area",
        "history_detail.study_area_id",
        "study_area.study_area_id"
      )
      .innerJoin(
        "sfa.program",
        "history_detail.program_id",
        "program.program_id"
      )
