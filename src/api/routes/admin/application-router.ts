...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (newRow && newRow.length == 1) {
      ...
      return res.json({
        messages: [
          { text: "This record appears to already exist", variant: "error" },
        ],
      });
    }
  }
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (count == 0) {
      let tries = 5;
      let nextId = (
        await db("sfaadmin.history_detail")
          .max("history_detail_id as max")
          .first()
      )?.max;
      ...
      while (tries > 0) {
        ...
        try {
          let newRow = await db("sfaadmin.history_detail")
            .insert(newApp)
            .returning("*");
      ...
      return res.json({
        messages: [{ text: "Failed to create application", variant: "error" }],
      });
    } else {
      return res.json({
        messages: [
  ...
  async (req: Request, res: Response) => {
    ...
    let application = await db("sfa.application")
      .select(
        "sfa.application.*",
        db.raw(
          "sfa.fn_get_prev_pre_leg_weeks(sfa.application.student_id, sfa.application.id) AS prev_pre_leg_weeks"
        ),
        db.raw(
          "sfa.fn_get_funded_years_used_preleg_chg(sfa.application.student_id, sfa.application.id) AS funded_years_used_preleg_chg"
        )
      )
      .where({ id })
      .first();
    ...
    if (application) {
      let student = await db("sfa.student")
        ...
        .first();
      application.incomes = await db("sfa.income").where({
        application_id: id,
      });
      application.expenses = await db("sfa.expense").where({
        application_id: id,
      });
      ...
      application.parent_dependents = await db("sfa.parent_dependent")
        .where({ application_id: id })
        .orderBy("birth_date");
      application.requirements = await db("sfa.requirement_met")
        .where({ application_id: id })
        .orderBy("completed_date");
      application.other_funding = await db("sfa.agency_assistance")
        .where({ application_id: id })
        .orderBy("id");
      application.yea = await db("sfa.yea")
        .where({ yukon_id: student.yukon_id })
        .orderBy("school_year");
      ...
      application.parent2 = await db("sfa.person")
        .where({ id: application.parent2_id })
        .first();
      application.spouse_info = await db("sfa.person")
        .where({ id: application.spouse_id })
        .first();
      application.agencies_assistance = await db("sfa.agency_assistance").where(
        { application_id: application.id }
      );
      application.courses_enrolled = await db("sfa.course_enrolled").where({
        application_id: application.id,
      });

      for (let dep of application.parent_dependents) {
        if (dep.birth_date)
          dep.birth_date = moment(dep.birth_date)
            .add(7, "hours")
            .format("YYYY-MM-DD");
      }
      ...
      for (let dep of application.requirements) {
        if (dep.completed_date)
          dep.completed_date = moment(dep.completed_date)
            .add(7, "hours")
            .format("YYYY-MM-DD");
      }
      ...
      for (let dep of application.funding_requests) {
        if (dep.received_date)
          dep.received_date = moment(dep.received_date)
            .add(7, "hours")
            .format("YYYY-MM-DD");

        if (dep.status_date)
          dep.status_date = moment(dep.status_date)
            .add(7, "hours")
            .format("YYYY-MM-DD");
      }
      if (student) {
        ...
        for (let dep of student.dependents) {
          let elg = await db("sfa.dependent_eligibility")
            .where({ application_id: id, dependent_id: dep.id })
            .first();
          dep.eligibility = elg || {};
        ...
        student.applications = await db("sfa.application")
          .innerJoin(
            ...
            "application.institution_campus_id",
            "institution_campus.id"
          )
          .innerJoin(
            "sfa.institution",
            "institution_id",
        ...
        if (student.birth_date) {
          student.birth_date = moment(student.birth_date)
            .utc(false)
            .format("YYYY-MM-DD");
        }
...
applicationRouter.put(
  ...
  async (req: Request, res: Response) => {
    ...
    db("sfa.application")
      ...
      .then((result: any) => {
        res.json({
          messages: [{ variant: "success", text: "Application saved" }],
        });
      })
      .catch((err: any) => {
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const application: any = await db("sfa.application")
      ...
      if (application) {
        ...
        if (checkIsActive?.is_active) {
          ...
          res.json({
            messages: [
              { variant: "error", text: "Request Type is not active" },
            ],
          });
        }
      }
      ...
      return res.json({
        messages: [{ text: "Application does not exist", variant: "error" }],
      });
    } catch (error) {
      return res.json({
        messages: [
          { text: "Failed to insert Funding Request", variant: "error" },
        ],
      });
    }
  }
...
applicationRouter.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res.json({
        messages: [
          { text: "Failed to update Funding Request", variant: "error" },
        ],
      });
    }
  }
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return deleteRecord > 0
        ? res
            .status(202)
            .send({ messages: [{ variant: "success", text: "Removed" }] })
        : res.status(404).send({
            messages: [{ variant: "error", text: "Record does not exits" }],
          });
    } catch (error: any) {
      console.log(error);
      ...
      if (error?.number === 547) {
        return res.status(409).send({
          messages: [
            {
              variant: "error",
              text: "Cannot be deleted because it is in use.",
            },
          ],
        });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        return res.json({
          messages: [{ variant: "error", text: "The value is required" }],
        });
      }
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (application) {
        const resInsert = await db("sfa.parent_dependent").insert({
          ...data,
          application_id,
        });
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!verifyRecord) {
        return res
          .status(404)
          .send({ wasDelete: false, message: "The record does not exits" });
      }
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!types[typeId]) {
        return res.json({
          messages: [{ variant: "error", text: "type Id is required" }],
        });
      }

      if (!Object.keys(data).length) {
        return res.json({
          messages: [{ variant: "error", text: "data is required" }],
        });
      }
      ...
      await db.transaction(async (trx) => {
        ...
        if (resInsert) {
          ...
          return res.json({
            messages: [{ variant: "success", text: "Inserted" }],
          });
        } else {
          res.json({
            messages: [{ variant: "error", text: "Failed to update" }],
          });
        }
      });
    } catch (error) {
      console.log(error);
      return res.json({
        messages: [{ variant: "error", text: "Save failed" }],
      });
    }
  }
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (personAddressId) {
        ...
        await db.transaction(async (trx) => {
          const [resInsert] = await Promise.all([
            trx("sfa.person").insert({ first_name: "" }).returning("*"),
          ]);
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!data?.agency_id) {
        return res.json({
          messages: [{ variant: "error", text: "Agency Id is required" }],
        });
      }
      ...
      const verifyAgency = await db("sfa.agency_assistance").where({
        application_id,
      ...
      if (verifyAgency?.length) {
        return res.json({
          messages: [{ variant: "error", text: "The Agency already exists" }],
        });
      }

      const resInsert = await db("sfa.agency_assistance").insert({
        ...data,
        application_id,
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (data?.description === "" || data?.description === null) {
        return res.json({
          messages: [{ variant: "error", text: "Description is required" }],
        });
      }
      ...
      if (!data?.instruction_type_id) {
        return res.json({
          messages: [
            { variant: "error", text: "Instruction Type is required" },
          ],
        });
      }

      const resInsert = await db("sfa.course_enrolled").insert({
        ...data,
        application_id,
      });
...
applicationRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const resInsert = await db("sfa.income").insert({
        ...data,
        application_id,
      });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (Object.keys(data).some((k) => k === "income_type_id")) {
        if (
          data?.income_type_id === "" ||
          data?.income_type_id === null ||
          isNaN(data?.income_type_id)
        ) {
          return res.json({
            messages: [{ variant: "error", text: "Income Type is required" }],
          });
        }
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!data?.period_id || !Number(data.period_id)) {
        return res.json({
          messages: [{ variant: "error", text: "Period is required" }],
        });
      }

      const resInsert = await db("sfa.expense").insert({
        ...data,
        application_id,
      });
